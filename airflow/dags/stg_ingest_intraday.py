...
dag = DAG(
    'integration_ingest_intraday',
    default_args=default_args,
    description='Run the integration layer of Acura dbt data models',
    schedule='0 0 * * *',
    start_date=datetime(2025, 6, 12),
    catchup=False,
    tags=["acura","integration","ingest","intraday"]
)

dbt_run_itg_layer = create_dbt_exec_operator(
    task_id='dbt_run_itg_layer',
    dbt_command = 'run --profiles-dir /usr/app/dbt/profiles --select integration.*'
)

dbt_test_itg_layer = create_dbt_exec_operator(
    task_id='dbt_test_itg_layer',
    dbt_command = 'test --profiles-dir /usr/app/dbt/profiles --select integration.*'
)
...
start >> dbt_run_itg_layer >> dbt_test_itg_layer >> end
